test_mpi <- function(script, procs = 2) {
  ...
  suppressWarnings(
    system2("mpiexec", sprintf(" -n %d Rscript -e %s", procs, script),
...
test_script <- function(n_particles, n_streams, out_file,
  ...
  "dustmpi:::simulate_model_to_file(list(gamma = %s, beta = %s, freq = %s),
      c(%s, %s, %s, 0, 0), %s, %s,
      dust::dust_rng_pointer$new(42, n_streams = %s), '%s', TRUE)",
      gamma, beta, freq, Si, Ii, Ri, time, n_streams, n_particles, out_file))
}
...
test_that("MPI Sim gives same result as single-processor", {
  ...
  script <- test_script(10, 10, tmp, pars$gamma, pars$beta, pars$freq, y[1], y[2], y[3], end_time)
  test_mpi(script, 10)
  res2 <- readRDS(tmp)
